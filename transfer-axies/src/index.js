...
const transferAxies = async () => {
  ...
  for (const account in transfers) {
    ...
    try{
      ...
      if(web3.eth.accounts.privateKeyToAccount(privateKey).address !== web3.utils.toChecksumAddress(account.replace("ronin:", "0x"))) {
      ...
      console.log(`Transfer failed for ${account} due to the following error: ${e}`)
...
const batchTransferAxie = async (
  ...
  try {
    ...
    if (await axieContract.methods.isApprovedForAll(accountAddress,BATCH_ADDRESS).call()!==true) {
    ...
    const estimatedGas = await batchContract.methods
      ['safeBatchTransfer(address,uint256[],address[])'](AXIE_ADDRESS, axieIds, recipientAddresses)
